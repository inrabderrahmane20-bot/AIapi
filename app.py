...
def cached_get(url: str, params: dict = None, headers: dict = None, ttl: int = config.CACHE_TTL):
    """
    Performs a GET and caches serializable data.
    """
...
class MapProvider:
    ...
    def generate_static_map_url(self, coordinates: Optional[Dict[str, float]], width: int = 600, height: int = 400) -> Optional[str]:
        if not coordinates or not self._validate_coordinates(coordinates):
            # Return a placeholder static map
            return "https://via.placeholder.com/600x400/4a5568/ffffff?text=Map+Unavailable"
        ...
        # OpenStreetMap static via staticmap.openstreetmap.de
        try:
            zoom = 12
...
class CityDataProvider:
        ...
        try:
            ...
            if len(images) < limit:
                ...
                if getattr(resp, "status_code", 0) == 200:
                    ...
                    if file_titles:
                        batch_size = 8
                        for i in range(0, min(len(file_titles), 24), batch_size):
    ...
    def get_city_preview(self, city_name: str) -> Dict:
        ...
        preview = {
            ...
            "has_details": False,
            "image": {"url": "https://via.placeholder.com/400x300/4a5568/ffffff?text=City+Image", "source": "placeholder"},
            "tagline": None,
            "tagline_source": None
        }
    ...
    def get_city_details(self, city_name: str) -> Dict:
        details = {
            ...
            "image": {"url": "https://via.placeholder.com/800x600/4a5568/ffffff?text=City+Image", "source": "placeholder"},
            "static_map": None,
            "tagline": None,
            "tagline_source": None
        }
...
# Keep your original WORLD_CITIES data here - I'm just showing a few examples
WORLD_CITIES = [
    # EUROPE 
...
def load_all_cities():
    ...
    def load_task():
        ...
        try:
            with ThreadPoolExecutor(max_workers=min(16, max(2, len(WORLD_CITIES)))) as executor:
                ...
                for i, future in enumerate(as_completed(futures)):
                    ...
                    try:
                        ...
                        if (i + 1) % 20 == 0:
                            loaded_with_images = sum(1 for city in loaded_data if city.get("image", {}).get("url") != "https://via.placeholder.com/400x300/4a5568/ffffff?text=City+Image")
            ...
            cities_with_images = sum(1 for city in ALL_CITIES_DATA if city.get("image", {}).get("url") != "https://via.placeholder.com/400x300/4a5568/ffffff?text=City+Image")
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    has_image = result.get("image", {}).get("url") != "https://via.placeholder.com/400x300/4a5568/ffffff?text=City+Image"
...
def serve_frontend():
    return jsonify({
        "message": "Traveltto API",
        "status": "healthy",
        "version": "1.0.0",
        "website": "https://www.traveltto.com",
        "endpoints": {
            "/api/health": "Health check",
            "/api/cities": "Get all cities",
            "/api/cities/{city}": "Get city details",
            "/api/search?q={query}": "Search cities",
...
def health():
    ...
    return jsonify({
        ...
        "cache_size": len(LOCAL_CITY_CACHE),
        "environment": "production" if not config.FLASK_DEBUG else "development"
    })
...
def clear_cache():
    try:
        city_name = request.json.get("city") if request.is_json else None
        if city_name:
...
if __name__ == "__main__":
    ...
    logger.info(f"🚀 Starting Traveltto API with {len(WORLD_CITIES)} cities")
    logger.info(f"🌍 Website: https://www.traveltto.com")
